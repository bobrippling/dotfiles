" This is taken from https://github.com/romainl/vim-qf, but only the filter part
"
...
function! s:FilterList(pat, reject)
...
command! -range -nargs=1 -bang Filter call s:FilterList(<q-args>, expand("<bang>") == "!" ? 1 : 0)

" keep entries matching the argument
" usage:
"   :Keep foo
command! -range -nargs=? Keep call s:FilterList(<q-args>, 0)

" reject entries matching the argument
" usage:
"   :Reject foo
command! -range -nargs=? Reject call s:FilterList(<q-args>, 1)

" restore the location/quickfix list
" usage:
"   :Restore
command! -bar Restore call s:RestoreList()
